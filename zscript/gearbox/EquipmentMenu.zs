Class gb_equipmentmenu
{
	...
	{
		if(token.size() > 0)
			return token[clamp(helditems[mSelectedIndex],0,token.size() - 1)];
		return "";
	}
	...
	private void Load()
	{
		for(int i = 0; i < AllClasses.size(); i++)
		{
			if(AllClasses[i] is "equipmentCard")
			{
				...
				if(eq)
					eq.InfoFiller(tags,token,ownedtoken,ammotype,img,scalex,scaley);
			}
		}
	}
	...
	array <string> tags;
	array <string> token;
	array <string> ownedtoken;
	array <string> ammotype;
	array <string> img;
	array <double> scalex;
	array <double> scaley;
